...
files.forEach(file => {
  ...
  // Fix pattern with template literals: toast.error(error.message || `text ${var}`)
  const pattern = /toast\.error\(error\.message \|\| `([^`]+)`\)/g;
  content = content.replace(pattern, (match, template) => {
    return `toast.error(error instanceof Error ? error.message : \`${template}\`)`;
  });
