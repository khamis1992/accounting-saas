...
// Test suite for the accounting system with authentication
test.describe('Accounting System Button Tests with Authentication', () => {
  ...
  test('should test form submissions and button clicks', async ({ page }) => {
    // Test navigating to a form and interacting with buttons
    await page.click('text=Sales');
    await page.click('text=Customers');
    await page.waitForURL('**/customers');
    ...
    const dialogTitle = page.locator('h2:has-text("Add Customer")');
    await expect(dialogTitle).toBeVisible();
    
    // Test that the form elements are present
    const codeInput = page.locator('input[placeholder="Customer code"]');
    await expect(codeInput).toBeVisible();
    
    const nameEnInput = page.locator('input[placeholder="Customer name in English"]');
    await expect(nameEnInput).toBeVisible();
    
    const nameArInput = page.locator('input[dir="rtl"]');
    await expect(nameArInput).toBeVisible();
