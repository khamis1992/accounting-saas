...
import { Plus, Search, Eye, Edit, Trash2, Send, Check, Upload } from 'lucide-react';
...
export default function JournalsPage() {
  ...
  const handleCreate = () => {
    router.push('/en/accounting/journals/new');
  };

  const handleView = (journal: Journal) => {
    router.push(`/en/accounting/journals/${journal.id}`);
  };
  ...
  const handleEdit = (journal: Journal) => {
    ...
    router.push(`/en/accounting/journals/${journal.id}/edit`);
  };
  ...
  const handleApprove = async (journal: Journal) => {
    ...
    try {
      ...
      toast.error(error.message || 'Failed to approve journal');
    } finally {
      setActionLoading(null);
    }
  };
                ...
                <TableBody>
                  {filteredJournals.map((journal) => (
                    <TableRow key={journal.id}>
                      ...
                      <TableCell>
                        {journal.description_en || journal.description_ar}
                      </TableCell>
