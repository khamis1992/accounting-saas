...
export default function InvoicesPage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge variant={variants[status] || "secondary"}>{t(`statuses.${status}` as any)}</Badge>
    );
  };
    ...
    <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-zinc-600 dark:text-zinc-400">Manage your customer invoices</p>
          </div>
              ...
              <TableBody>
                {filteredInvoices.map((invoice) => (
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell className="text-right">QAR {invoice.vat.toLocaleString()}</TableCell>
                    <TableCell className="text-right">
